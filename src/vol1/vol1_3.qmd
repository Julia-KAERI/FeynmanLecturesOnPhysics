...
조화 진동자는 물리학 뿐만 아니라 다른 많은 분야에서도 유사한 현상이 나타난다. 원래 조화진동자는 용수철에 매달린 무게의 운동, 혹은 진동의 폭이 작은 진자를 기술하는 모델로서 특정 미분 방정식으로 기술된다. 이 방정식은 물리학 을 포함한 여러 분야에서 반복적으로 등장하며, 수많은 현상의 일부이므로 면밀히 살펴볼 가치가 있다. 이 방정식과 관련된 현상에는 스프링에 매달린 질량의 진동, 전기 회로에서 전하가 앞뒤로 흐르는 진동, 음파를 발생시키는 튜닝 포크의 진동, 빛 파동을 생성하는 원자 내 전자의 유사한 진동, 온도 조절을 시도하는 서보시스템의 작동 방정식, 화학 반응에서의 복잡한 상호작용, 식량 공급 및 박테리아가 생성하는 독과의 상호작용에서 박테리아 군집의 성장, 여우가 풀을 먹는 토끼 등 On; 이러한 모든 현상은 서로 매우 유사한 방정식을 따르며, 이것이 우리가 기계식 발진기를 이렇게 자세히 연구하는 이유입니다. 그 방정식은 상수 계수를 갖는 선형 미분 방정식이라고 합니다.
