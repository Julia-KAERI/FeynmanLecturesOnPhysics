...
% WIRE B FIELD 3D
\begin{tikzpicture}[z={(0.8,0.28)},x={(0.58,-0.45)}, scale=2]
  \def\L{6}
  \def\W{0.30}
  \def\R{0.9}
  \def\ang{-35}
  ...
  \foreach \i [evaluate={\x=(\i-\NB/2-0.5)*\L/\NB;}] in {1,...,\NB}{
    ...
    \draw[BFieldLine=1] (0,0,\x)++(\ang-181:\R) arc (\ang-181:\ang+1:\R) --++ (45:0.001*\R);
  }
  
  % WIRE
  \draw[metal] (0,0,-\L/2)++(120:\W/2) --++ (0,0,\L) arc (120:-60:\W/2) --++ (0,0,-\L) arc (-60:120:\W/2);
  \draw[metal] (0,0,-\L/2) circle (\W/2);
  \draw[current] (0.3*\R,-0.12*\R,-0.2*\L) --++ (0,0,-0.2*\L) node[below=4,right] {$I$};
  ...
  \node[Bcol] at (1*\R,-0.4*\R, 0.03*\L) {$\boldsymbol{B}$}; %++(140:1.3*\R)
  
  \draw[->] (0, 0, 0) -- (-\R/1.414, \R/1.414, 0) node [midway, right] {$\boldsymbol{r}$};
\end{tikzpicture}
