...
\begin{tikzpicture}
  ...
  \begin{scope}  
    \draw[bath]
        (C)++(-.5*\W,0) rectangle ++(\W,-\H) node[midway,align=center] {$T_2$};
    ...
    %     to[out=180,in=-90] (.5+.1*\W,.2+.05*\L) -- (.5+.1*\W,\L/2);
        ...
        \draw[C1]
        (-0.2*\W, -0.4) rectangle ++(0.4*\W, 0.8) node[midway,scale=0.9] {$A$};
    
        \draw[source]
        (H)++(-.5*\W,0) rectangle ++(\W,\H) node[midway,align=center] {$T_1$};
    
        \draw[very thick, <-] (0, -\L/2) -- node[midway, fill=white] {$Q_1-W$} (0, -0.4);   
        \draw[very thick, <-] (0, 0.4) -- node[midway, fill=white] {$Q_1$} (0, \L/2);  
        \draw[very thick, ->] (0.2*\W, 0) -- node[midway, fill=white] {$W$} (0.65*\W, 0);    
  \end{scope}
  ...
  \begin{scope}[xshift=130]  
    ...
    %     to[out=180,in=-90] (.5+.1*\W,.2+.05*\L) -- (.5+.1*\W,\L/2);
        ...
        \draw[C1]
        (-0.2*\W, -0.4) rectangle ++(0.4*\W, 0.8) node[midway,scale=0.9] {$B$};
    
        \draw[source]
        (H)++(-.5*\W,0) rectangle ++(\W,\H) node[midway,align=center] {$T_1$};
    
        \draw[very thick, <-] (0, -\L/2) -- node[midway, fill=white] {$Q_B$} (0, -0.4);   
        \draw[very thick, <-] (0, 0.4) -- node[midway, fill=white] {$Q_1$} (0, \L/2);  
        \draw[very thick, ->] (0.2*\W, 0) -- node[midway, fill=white] {$W'$} (0.65*\W, 0);    
  \end{scope}
